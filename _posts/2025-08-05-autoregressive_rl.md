...
In the **non-Markovian** setting, the next state, action, and reward may depend on the entire history, not just the current state and action. In **autoregressive RL**, the _sequence_ of actions and states is modeled as an autoregressive process over time (not over state dimensions).
...
By introducing a _null token_ for the next state, the policy is defined as the marginal over actions:
